...
function analyzeSemanticUnderstanding(
  ast: t.File,
  importMap: Map<string, string>,
  moduleName: string,
  symbolCount: number
  ...
  traverse(ast, {
    ImportDeclaration(path) {
      ...
      let usageType: "runtime" | "type" | "side_effect" = "runtime";
    ...
    FunctionDeclaration(path) {
      ...
      if (name.match(/^(get|post|put|patch|delete|fetch|request)/i)) {
        ...
        if (name.match(/^post/i)) method = "POST";
        else if (name.match(/^put/i)) method = "PUT";
        else if (name.match(/^patch/i)) method = "PATCH";
        else if (name.match(/^delete/i)) method = "DELETE";
...
export function analyzeCodeUnderstanding(
  ...
  const importMapForSemantic = new Map<string, string>();
  const semanticAnalysis = analyzeSemanticUnderstanding(
    ast,
    importMapForSemantic,
    moduleName,
    symbols.length
  );
